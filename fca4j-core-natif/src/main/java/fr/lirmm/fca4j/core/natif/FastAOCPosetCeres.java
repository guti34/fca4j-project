/*
 * Copyright (c) 2026 LIRMM — BSD 3-Clause License
 * See LICENSE file in the project root for full license text.
 */
package fr.lirmm.fca4j.core.natif;

import fr.lirmm.fca4j.algo.AOC_poset_Ceres;
import fr.lirmm.fca4j.algo.AbstractAlgo;
import fr.lirmm.fca4j.core.IBinaryContext;
import fr.lirmm.fca4j.core.natif.impl.NativeAOCPosetCeres;

/**
 * Point d'entrée public pour l'AOC-poset Ceres.
 *
 * <ul>
 *   <li><b>Natif disponible</b> → {@link NativeAOCPosetCeres} : calcul complet
 *       en C, sans clarification préalable.</li>
 *   <li><b>Repli</b> → {@link AOC_poset_Ceres} standard Java.</li>
 * </ul>
 */
public final class FastAOCPosetCeres {

    static {
        NativeBridge.isAvailable();
    }

    private FastAOCPosetCeres() {
    }

    /**
     * Crée une instance optimisée de Ceres.
     *
     * @param context contexte binaire d'entrée
     * @return instance prête à exécuter (appeler {@code run()} puis {@code getResult()})
     */
    public static AbstractAlgo create(IBinaryContext context) {
        if (NativeBridge.isAvailable()) {
            return new NativeAOCPosetCeres(context);
        } else {
            return new AOC_poset_Ceres(context);
        }
    }

    /**
     * Indique quel backend est actif.
     */
    public static String activeBackend() {
        return NativeBridge.isAvailable()
                ? "native (C)"
                : "java (AOC_poset_Ceres standard)";
    }
}
