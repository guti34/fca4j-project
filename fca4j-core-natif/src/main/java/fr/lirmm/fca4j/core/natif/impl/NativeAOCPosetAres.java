...
/**
 * Implémentation native de l'AOC-poset Ares.
 *
 * <p>Délègue l'intégralité du calcul à {@code run_ares_flat} via
 * {@link NativeBridge#runAresFlat}. Le C construit l'ordre dans une structure
 * dédiée — extents denses, couvertures en listes d'entiers, marquages
 ...
 *
 * <p>Contrairement à Hermes et Pluton, le portage ne clarifie pas : Ares
 * travaille sur le contexte original, donc les indices du tableau plat sont
 * déjà ceux du contexte d'entrée, sans substitution.
 *
 ...
 * </pre>
 */
public class NativeAOCPosetAres implements AbstractAlgo {

    private final IBinaryContext matrix;
    private ConceptOrder order;

    public NativeAOCPosetAres(IBinaryContext matrix) {
        this.matrix = matrix;
    }
    ...
    public void run() {
        ...
        int[] flat = NativeBridge.runAresFlat(nObj, nAttr, mat);

        order = new ConceptOrder("AOCposetWithAres", matrix, getDescription());
    ...
    static byte[] buildMatrix(IBinaryContext ctx) {
        ...
        for (int o = 0; o < nObj; o++) {
            for (Iterator it = ctx.getIntent(o).iterator(); it.hasNext();) {
