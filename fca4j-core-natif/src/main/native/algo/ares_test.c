/*
 * ares_test.c — validation différentielle du portage C d'ARES
 * Copyright (c) 2026 LIRMM — BSD 3-Clause License
 *
 * Compare la sortie de run_ares_flat à celle de run_hermes_flat, qui construit
 ...
 *   gcc -std=c11 -O2 -Wall -Wextra -o ares_test \
...
static const char *compare_flat(const Flat *fa, const Flat *fh) {
    ...
    if (fa->N != fh->N) {
        snprintf(msg, sizeof(msg), "nombre de concepts : ares=%d hermes=%d", fa->N, fh->N);
        return msg;
    }
    if (fa->E != fh->E) {
        snprintf(msg, sizeof(msg), "nombre d'aretes : ares=%d hermes=%d", fa->E, fh->E);
        return msg;
    }
    int N = fa->N;
    if (N == 0) return NULL;
        ...
        for (int j = i + 1; j < N; j++)
            if (strcmp(sa + (size_t)i * SIGCAP, sa + (size_t)j * SIGCAP) == 0) {
                snprintf(msg, sizeof(msg),
                         "signature dupliquee cote ares (concepts %d et %d) : %s",
    ...
    for (int i = 0; i < E; i++) {
        if (ea[2 * i] != eh[2 * i] || ea[2 * i + 1] != eh[2 * i + 1]) {
            snprintf(msg, sizeof(msg),
                     "arete divergente au rang %d : ares (%d,%d) hermes (%d,%d)",
...
static int run_one(const signed char *m, int n, int p, int verbose) {
    ...
    int la = 0, lh = 0;
    int *ba = run_ares_flat(c1, &la);
    int *bh = run_hermes_flat(c2, &lh);
    int ok = 1;
    if (!ba || !bh) {
        printf("  ECHEC : sortie nulle (ares=%p hermes=%p)\n", (void*)ba, (void*)bh);
        ok = 0;
    } else {
...
int main(int argc, char **argv) {
    ...
    printf("%s\n", failures == 0 ? "ares : conforme a hermes" : "ares : DIVERGENCES");
