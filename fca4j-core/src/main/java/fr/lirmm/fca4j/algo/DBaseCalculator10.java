...
public class DBaseCalculator10 implements AbstractAlgo<List<Implication>> {
	ISet supportBidon;
	...
	public DBaseCalculator10(IBinaryContext context) {
...
/*	public Set<ISet> computeMinimalGenerators2(int target) {
		Set<ISet> result = new HashSet<>();

		// Génération de tous les candidats potentiels
		for (ISet candidate : generateAllSubsetsWithout(target)) {
			if (!RuleUtilities.computeClosure(candidate, dqBasis).contains(target))
				continue;
...
*/
	/**
	 * Retourne les générateurs minimaux de l'attribut cible.
	 *
	 * @param allAttributes Ensemble total des attributs (sous forme de BitSet).
	 * @param targetIndex   Index de l'attribut cible b.
	 * @return Ensemble des générateurs minimaux sous forme de BitSet.
	 */
	public Set<ISet> computeMinimalGenerators(int target) {
		Set<ISet> result = new HashSet<>();

		ISet searchSpace = context.getFactory().createSet();
		searchSpace.fill(context.getAttributeCount());
		searchSpace.remove(target); // On ne veut pas b dans les prémisses

		ISet current = context.getFactory().createSet();
		explore(current, searchSpace.iterator(), searchSpace, target, result);
	...
	private void explore(ISet current, Iterator<Integer> iterator, ISet available, int target, Set<ISet> result) {
		if (RuleUtilities.computeClosure(current, dqBasis).contains(target)) {
			...
			return; // On n'explore pas les supersets
		}
	...
	protected List<ISet> generateAllSubsetsWithout(int attr) {
		List<ISet> subsets = new ArrayList<>();
		int N = context.getAttributeCount();
		for (int i = 0; i < (1 << N); i++) {
