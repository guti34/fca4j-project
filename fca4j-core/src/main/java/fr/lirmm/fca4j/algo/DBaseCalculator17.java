...
public class DBaseCalculator17 implements AbstractAlgo<List<Implication>> {
	...
	public DBaseCalculator17(IBinaryContext context) {
	...
	public List<Implication> computeDBasis() {
		...
		int redundants=0;
		ArrayList<Implication> dBasisFiltered=new ArrayList<>();
		for (Implication impl : dBasis) {
		    ISet premise = impl.getPremise();
		    int conclusion = impl.getConclusion().first();

		    if (!engine.implies(premise, conclusion)) {
		    	dBasisFiltered.add(impl);
		        engine.add(impl);
		    }
		    else redundants++;
		    if(redundants%100==0) System.out.println("redundants="+redundants);
		}		
		chrono.stop("filter redundancy");
		System.out.println("redundants="+redundants);
	...
	public class ClosureEngine {
	    ...
	    public boolean implies(ISet premise, int target) {
	        ...
	        while (!frontier.isEmpty()) {
	            ...
	            for (Implication impl : candidates) {
	                if (closure.containsAll(impl.getConclusion())) continue;

	                ISet premiseOfImpl = impl.getPremise();
	                if (closure.containsAll(premiseOfImpl)) {
	                    int conclusion = impl.getConclusion().first();
